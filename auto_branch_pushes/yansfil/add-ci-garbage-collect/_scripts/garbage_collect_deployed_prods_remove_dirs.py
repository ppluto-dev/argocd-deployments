...
print(subprocess.run('git fetch --prune'.split(), stdout=subprocess.PIPE).stdout.decode('utf-8')) 

git_remote_branches_raw = subprocess.run('git branch --remotes'.split(), stdout=subprocess.PIPE).stdout.decode('utf-8')
...
def should_keep(dir):
    for git_remote_branch_name in git_remote_branch_names:
        ...
        if dir.startswith(git_remote_branch_name) or git_remote_branch_name.startswith(dir):
            return True
    return False
...
for rootdir, dirs, files in os.walk(prod_branches_directory):
    for subdir in dirs:
        ...
        prod_branch_dirs.append(full_path[len(prod_branches_directory):].lstrip('/') + '/')
